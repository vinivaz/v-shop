...
import { canUserRate } from "@/lib/api/server/products";


async function checkIfUserCanRateProduct(productId: string):Promise<boolean>{
  try{
    const {data, error} = await canUserRate(productId);
    console.log(error)
...
export default async function Product({
  ...
  const ratingAllowed = await checkIfUserCanRateProduct(product!.id)
          ...
          <div className="flex text-sm my-2">
            {product.averageRating != null ?
              ...
              >
                ...
                <span>{`${product.averageRating} (${product.ratingCount}) `}</span>
              </div>
            :
              <p>Nenhuma avaliação ainda.</p>
            }

            {ratingAllowed && <RatingPopOver
              product={product}
            />}
